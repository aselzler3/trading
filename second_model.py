# -*- coding: utf-8 -*-
"""
Created on Fri Feb  2 22:08:46 2018
...
class Network(object):
    ...
    def SGD(self, data, eta, mini_batch_size):
        ...
        result=[]
        n=len(data)
        for i in range(mini_batch_size,len(data)):
            if i%500==0:
                print(i)
            result.append(self.feedforward(data[i][0]))
            self.update_mini_batch([(data[i-(mini_batch_size-j-1)][0],data[i-(mini_batch_size-j-1)][1]) for j in range(mini_batch_size)],eta)
        return result
...
n_days=7 #days given to each input(X)
timespan=1 #number of days for moving average
mbs=4 #mini batch size for gradient descent
MA_data=[]
for i in range(timespan,len(P)):
    MA_data.append(np.mean(np.array(P[i-timespan:i])))
P=np.array(MA_data)
...
predictions=model.SGD(data,eta=2,mini_batch_size=mbs)
predictions=[predictions[i][0][0] for i in range(len(predictions))]
Y=[x[1] for x in data[mbs:]]
...
for i in range(100,len(Y)):
    moving_R.append(R_squared(np.array(Y[i-100:i]),np.array(predictions[i-100:i])))
plt.plot(moving_R)
plt.show()
